...
def make_iemocap_df(root_path: Path) -> pd.DataFrame:
    all_files = list(root_path.rglob("**/*.wav"))
    all_files = [f for f in all_files if "._" not in f.stem]
    speakers = [f.stem.split("_")[-1].split(".")[0] for f in all_files]
...
def main(args):
    ...
    if args.dataset == "librispeech":
        df = make_librispeech_df(Path(args.librispeech_path))
...
def path2pools(
    ...
    uttrs_from_same_spk = list(df[df.speaker == speaker].path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", required=True, type=str, help="librispeech or iemocap"
    )
    parser.add_argument("--librispeech_path", required=False, type=str)
    parser.add_argument("--iemocap_path", required=False, type=str)
